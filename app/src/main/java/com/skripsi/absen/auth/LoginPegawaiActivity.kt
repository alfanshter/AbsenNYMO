...
import com.skripsi.absen.karyawan.KaryawanHomeActivity
import com.skripsi.absen.utils.Constant
import com.skripsi.absen.utils.CustomProgressDialog
import kotlinx.android.synthetic.main.activity_login_pegawai.*
import kotlinx.android.synthetic.main.activity_login_pegawai.btn_login
...
class   LoginPegawaiActivity : AppCompatActivity() {
    ...
    fun login_username() {
        ...
        if (password.isNotEmpty()  && username.isNotEmpty()) {
            ...
            val docref = firestore.collection(Constant.akunpegawai).whereEqualTo("username", username)
...
/*            auth.signInWithEmailAndPassword(email, password)
    ...
    private fun gettoken() {
        FirebaseMessaging.getInstance().token.addOnCompleteListener(OnCompleteListener { task ->
            ...
            if (token != null) {
                val req =
                    firestore.collection(Constant.akunpegawai).document(auth.currentUser!!.uid)
