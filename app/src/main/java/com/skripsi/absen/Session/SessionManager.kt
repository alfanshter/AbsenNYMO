...
class SessionManager(private val context: Context?) {
    ...
    private val isloginpimpinan = "loginpimpinan"
    fun setLoginpimpinan(check: Boolean){
        editor?.putBoolean(isloginpimpinan,check)
        editor?.commit()
    }

    fun getLoginpimpinan():Boolean?
    {
        return Pref?.getBoolean(isloginpimpinan,false)
    }
